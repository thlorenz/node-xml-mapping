			...
			if (type1 == 'object') {
				...
				if (type2 == 'object') {
					val.forEach(function(item, index) {
							if (key == '__element' || key == '__e' || key == '#element') {
								parse(item);
							}
							else {
								xw.startElement(getname(key));
				...
				if (key == '__text' || key == '__t' || key == '#text') {
					xw.text(getval(val));
				}
				else if (key == '__comment' || key == '__c' || key == '#comment') {
					xw.startComment().text(getval(val)).endComment();
				}
				else if (key == '__cdata' || key == '__cd' || key == '#cdata' || key == '#cd') {
